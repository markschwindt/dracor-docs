...
An example: Goethe's [»Urfaust«](https://dracor.org/id/ger000539) was written between 1772 and 1775, but not printed until 1887 and first performed only in 1918. The »Normalised Year« for this play would thus be 1775.

As DraCor is an open-source project, you may have a look at the underlying XQuery algorithm [here](https://github.com/dracor-org/dracor-api/blob/88c5d2951d15fa6c6e1d8790d9b514a5e8df65bb/modules/util.xqm#L383).

Please note that you do not necessarily have to work with our »Normalised Year«, but that this is just an offer for quick and reliable chronological overviews and charts. All other temporal information on the creation, publication and first performance of a play is still available both in the TEI documents and via our API.
...
Requests using the old-style URLs without a version prefix are redirected to the legacy version. For instance, `https://dracor.org/api/info` now redirects to `https://dracor.org/api/v0/info`. Therefore, if you have not adjusted your scripts, they should continue to work as long as they follow the redirect.

If this is not the case, you may simply add the `/v0` to your API URLs. For example, if you have used the API base URL `https://dracor.org/api`, change it to `https://dracor.org/api/v0`.
